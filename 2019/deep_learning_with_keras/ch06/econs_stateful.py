...
# MinMaxScalerは、min, maxの値を用いてデータを正規化する
scaler = MinMaxScaler(feature_range=(0, 1), copy=False)
data = scaler.fit_transform(data)

NUM_TIMESTEPS = 20

X = np.zeros((data.shape[0], NUM_TIMESTEPS)) # data.shape[0] - NUM_TIMESTEPS - 1にすべきでは？
Y = np.zeros((data.shape[0], 1))
for i in range(len(data) - NUM_TIMESTEPS - 1):
    X[i] = data[i:i + NUM_TIMESTEPS].T # .Tで転置行列を求めている
    Y[i] = data[i + NUM_TIMESTEPS] # 1が不要な気が
